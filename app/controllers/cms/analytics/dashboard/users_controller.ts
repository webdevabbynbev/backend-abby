...
export default class UsersController {
  private service = new DashboardUsersService()

  public async getTotalRegisterUser({ response }: HttpContext) {
    const total = await this.service.totalRegisteredActiveUsers()
  ...
  public async getTotalRegisterUserByPeriod({ response }: HttpContext) {
    const data = await this.service.registeredUsersByPeriod()
