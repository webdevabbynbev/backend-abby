...
import { createTagProduct } from '#validators/tag_product'
...
export default class TagProductsController {
    ...
    try {
      ...
      const tags = await TagProducts.query()
  ...
  public async store({ response, request, auth }: HttpContext) {
    try {
      const payload = await createTagProduct.validate(request.all())

      const tag: TagProducts = await TagProducts.create({
        ...payload,
  ...
  public async update({ response, params, request, auth }: HttpContext) {
    try {
      ...
      const tag: TagProducts | null = await TagProducts.query()
        .apply((query) => query.active())
        .where('slug', slug)
        .first()
  ...
  public async show({ response, params }: HttpContext) {
    try {
      ...
      const tag: TagProducts | null = (await TagProducts.findColumnWithSoftDelete('slug', slug)) as TagProducts
