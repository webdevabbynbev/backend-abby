...
export default class ProductsController {
    // ✅ Get products list (with review aggregate)
  public async get({ response, request }: HttpContext) {
    try {
      ...
      const productsQuery = await Product.query()
        ...
        .where('products.is_flashsale', '!=', 3)
        .if(name, (query) => {
          query.where('products.name', 'like', `%${name}%`)
        })
        .if(categoryType, (query) => {
          query.whereIn('products.category_type_id', categoryType)
        })
        .if(tagId, (query) => {
          query.where('products.tag_id', tagId)
        })
        .if(subTagId, (query) => {
          query.where('products.sub_tag_id', subTagId)
        })
